 ...
 */
static void button_3d_event_handler(lv_event_t *e) __attribute__((unused));
...
// ============================================================================
// Modal Gesture Support - Close modal on tap outside or swipe
// ============================================================================

/**
 * Simple tap-outside-to-close for modals
 * This allows users to dismiss modals by tapping the dimmed area behind them
 */
static void modal_tap_outside_handler(lv_event_t *e) {
    lv_event_code_t code = lv_event_get_code(e);
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        // Only close if the click was directly on the modal background (not on children)
        if (clicked == modal) {
            ESP_LOGI(TAG, "Modal: Tap outside detected - closing modal");
            lv_obj_del(modal);
        }
    }
}
...
/**
 * Add gesture support to a modal overlay
 * Allows tapping outside to close the modal
 */
static void add_modal_gesture_support(lv_obj_t *modal) {
    if (!modal) return;

    // Add click event to close when tapping outside (pass modal as user_data)
    lv_obj_add_event_cb(modal, modal_tap_outside_handler, LV_EVENT_CLICKED, modal);
    lv_obj_add_flag(modal, LV_OBJ_FLAG_CLICKABLE);

    ESP_LOGD(TAG, "Added gesture support to modal (tap outside to close)");
}
 ...
 */
static lv_obj_t* create_base_screen(const char *title, ui_page_t page, ui_footer_page_cb_t page_callback, lv_obj_t **footer_out) __attribute__((unused));
...
static char g_gps_url[128] = "http://192.168.68.51:10110";  // Default GPS URL (NMEA simulator)
...
static void config_gps_source_clicked(lv_event_t *e) {
    ...
    // Add gesture support (tap outside to close)
    add_modal_gesture_support(modal);
...
static void config_btn4_clicked(lv_event_t *e) __attribute__((unused));
...
lv_obj_t* create_config_screen(ui_footer_page_cb_t page_callback, lv_obj_t **footer_out, ui_styles_t* styles) {
    ...
    (void)grid_height;  // Reserved for future use
    ...
    // Create footer - keep it always visible on CONFIG screen (no auto-hide)
    lv_obj_t *footer = ui_footer_create(screen, PAGE_CONFIG, page_callback, styles);
    if (footer != NULL) {
        ui_footer_show(footer);
        lv_obj_move_foreground(footer);

        // Disable auto-hide timer on CONFIG screen so footer stays visible
        ui_footer_disable_auto_hide(footer);
        ESP_LOGI(TAG, "CONFIG screen: Auto-hide disabled - footer always visible");
    }
    if (footer_out != NULL) {
        *footer_out = footer;
    }
...
// Callback for GPS test back button
static void gps_test_back_clicked(lv_event_t *e) {
    lv_obj_t *prev_screen = (lv_obj_t *)lv_event_get_user_data(e);
    if (prev_screen) {
        lv_scr_load(prev_screen);
    }
}

static void tools_gpstest_clicked(lv_event_t *e) {
    // Use ERROR level to make sure this is visible
    ESP_LOGE(TAG, "");
    ESP_LOGE(TAG, "==============================================");
    ESP_LOGE(TAG, "GPS TEST BUTTON CLICKED!");
    ESP_LOGE(TAG, "==============================================");
    ESP_LOGE(TAG, "");
    ESP_LOGI(TAG, "Running GPS M8N diagnostics...");

    // Get current tools screen for back button
    lv_obj_t *tools_screen = lv_scr_act();

    // Create GPS test results screen
    lv_obj_t *test_screen = lv_obj_create(NULL);
    lv_obj_add_style(test_screen, ui_styles_get_screen_bg(g_screen_styles), 0);

    // Create header
    ui_header_create(test_screen, g_screen_styles);

    // Title
    lv_obj_t *title = lv_label_create(test_screen);
    lv_label_set_text(title, "GPS M8N TEST");
    lv_obj_add_style(title, ui_styles_get_title(g_screen_styles), 0);
    lv_obj_align(title, LV_ALIGN_TOP_MID, 0, HEADER_HEIGHT + 10);

    // Results panel with scrolling
    lv_obj_t *panel = lv_obj_create(test_screen);
    lv_obj_set_size(panel, 750, 300);
    lv_obj_align(panel, LV_ALIGN_CENTER, 0, 0);
    lv_obj_set_style_bg_color(panel, lv_color_hex(0x001122), 0);
    lv_obj_set_style_border_color(panel, lv_color_hex(0x00AA00), 0);
    ...
    // Results label
    lv_obj_t *results_label = lv_label_create(panel);
    lv_label_set_long_mode(results_label, LV_LABEL_LONG_WRAP);
    lv_obj_set_width(results_label, 720);
    lv_obj_set_style_text_color(results_label, lv_color_hex(0x00FF00), 0);
    lv_obj_set_style_text_font(results_label, &golostext_regular_16, 0);  // Use custom font
    lv_label_set_text(results_label, "Running GPS tests...\n\nPlease wait...");

    // Back button
    lv_obj_t *btn_back = lv_btn_create(test_screen);
    lv_obj_set_size(btn_back, 150, 50);
    lv_obj_align(btn_back, LV_ALIGN_BOTTOM_MID, 0, -20);
    apply_3d_button_style(btn_back, THEME_BTN_PRIMARY);
    lv_obj_add_event_cb(btn_back, gps_test_back_clicked, LV_EVENT_CLICKED, tools_screen);
    ...
    // Run GPS tests (this will take a few seconds)
    ESP_LOGI(TAG, "Starting GPS M8N test sequence...");

    // Build test results string
    char results[2048];
    int offset = 0;

    offset += snprintf(results + offset, sizeof(results) - offset,
                      "=== GPS M8N TEST RESULTS ===\n\n");

    // Test 1: I2C scan
    offset += snprintf(results + offset, sizeof(results) - offset,
                      "1. I2C Bus Scan:\n");
    bool gps_found = gps_test_scan_i2c();
    if (gps_found) {
        offset += snprintf(results + offset, sizeof(results) - offset,
                          "   PASS - GPS detected at 0x42\n\n");
    } else {
        offset += snprintf(results + offset, sizeof(results) - offset,
                          "   FAIL - GPS not found\n");
        offset += snprintf(results + offset, sizeof(results) - offset,
    ...
    if (gps_found) {
        ...
        offset += snprintf(results + offset, sizeof(results) - offset,
                          "   Fix: %s\n",
                          (ret == ESP_OK && gps_data.valid) ? "YES" : "NO");
    }
...
static lv_obj_t* create_tool_button(lv_obj_t *parent, const char *label, int x, int y,
                                    lv_event_cb_t callback) {
    // Log GPS Test button creation
    if (strstr(label, "GPS") != NULL) {
        ESP_LOGE(TAG, "Creating GPS Test button at x=%d, y=%d, callback=%p", x, y, callback);
    }
...
static void wifi_adhoc_clicked(lv_event_t *e) {
    ...
    // Add gesture support (tap outside to close)
    add_modal_gesture_support(adhoc_modal);
...
static void wifi_network_clicked(lv_event_t *e) {
    ...
    if (ssid) {
        ...
        // Add gesture support (tap outside to close)
        add_modal_gesture_support(wifi_password_screen);
...
static lv_obj_t* create_save_config_screen(lv_obj_t *tools_screen_ref) __attribute__((unused));
static lv_obj_t* create_save_config_screen(lv_obj_t *tools_screen_ref) {
    lv_obj_t *screen = lv_obj_create(NULL);
    lv_obj_add_style(screen, ui_styles_get_screen_bg(g_screen_styles), 0);
...
static lv_obj_t* create_load_config_screen(lv_obj_t *tools_screen_ref) __attribute__((unused));
static lv_obj_t* create_load_config_screen(lv_obj_t *tools_screen_ref) {
    lv_obj_t *screen = lv_obj_create(NULL);
    lv_obj_add_style(screen, ui_styles_get_screen_bg(g_screen_styles), 0);
...
static void display_menu_clicked(lv_event_t *e) __attribute__((unused));
...
static void display_info_clicked(lv_event_t *e) __attribute__((unused));
...
static void display_config_clicked(lv_event_t *e) __attribute__((unused));
...
static void display_mode_clicked(lv_event_t *e) __attribute__((unused));
...
static void test_led_clicked(lv_event_t *e) __attribute__((unused));
